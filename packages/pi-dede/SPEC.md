...
The config accepts `profiles.<profile>.model`, `profiles.<profile>.thinking`, `profiles.<profile>.env`, optional `profiles.<profile>.additionalArgs`, a top-level `additionalArgs` object mapping CLI flags to boolean or string values, and `context.forkMinTokens`/`context.forkMaxContextRatio`. The context defaults are 4,000 tokens and 0.7. `true` emits a flag, `false` omits it, and a non-empty string emits a flag/value pair. Extra arguments are inserted after pi-dede's built-in child options; the task remains an RPC stdin prompt. Positional prompts cannot be configured. Because an arbitrary child override can change prompt/model/tool fidelity, any configured override makes `auto` fall back to isolation. The profile-specific object replaces the shared object for that profile when present, including `{}`; a trusted project's top-level object replaces the global object and profile fields override corresponding global profile fields. These overrides are trusted configuration: conflicting child lifecycle, session, model, or tool flags are the configuration owner's responsibility. Environment maps contain portable string keys and string values. Session/delegation control names and process-startup variables (`PATH`, Node/Bun options, dynamic-loader variables, and `BASH_ENV`) are protected from environment overrides.
